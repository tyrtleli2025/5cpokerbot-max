...
class Player(Bot):
    ...
    def get_action(self, game_state, round_state, active):
        ...
        # ---------------------------------------------------------
        # POST-FLOP (Street 4+)
        # Fix: Don't fold everything. Bet draws and continue aggression.
        # ---------------------------------------------------------
        all_cards = my_cards + board_cards
        all_ranks = [rank_map[c[0]] for c in all_cards]
        all_suits = [c[1] for c in all_cards]
        
        # 1. Strong Hand: Pair or better
        is_strong = len(set(all_ranks)) < len(all_ranks)
        
        # 2. Draw: 3 to a flush (needs 4 total in this game variant usually, but 3 is a good start)
        suit_counts = {}
        for s in all_suits:
            suit_counts[s] = suit_counts.get(s, 0) + 1
        is_flush_draw = any(count >= 3 for count in suit_counts.values())

        # 3. Pot Odds / Aggression Check
        # If we raised pre-flop (indicated by a large pot already), we shouldn't give up easily.
        pot_total = my_pip + opp_pip
        is_continuation_bet_opportunity = (pot_total > 20)  # Rough heuristic: if pot is big, we likely raised
        ...
        if is_strong:
            ...
            # Semi-Bluff: Raise smaller or Call to see next card
            if RaiseAction in legal_actions:
                 min_raise, max_raise = round_state.raise_bounds()
                 # Raise 50% of min+max (a half-pot sized bet equivalent)
                 return RaiseAction(int((min_raise + max_raise) / 2))
            if CallAction in legal_actions:
                return CallAction()
            return CheckAction()

        # Weak hand, small pot -> Give up
        if CheckAction in legal_actions:
            return CheckAction()
        return FoldAction()
